...
void Sphere::draw() const {
  ...
  glMaterialf(GL_FRONT, GL_SHININESS, .4f * 128.f);
...
//      glVertex3f(p2.x + x, p2.y + y, p2.z + z);
      ...
      this->faces.push_back(Polygon(std::vector<Vertex>(
          {Vertex(points_count * (i + 1) + j + 1, 2),
           Vertex((j == points_count - 1 ? points_count * (i + 1) + 1 : points_count * (i + 1) + j + 2)),
           Vertex((j == points_count - 1 ? points_count * i + 1 : points_count * i + j + 2), 3)}),
                                    this->normals.size()));
...
//      glVertex3f(p2.x + x, p2.y + y, p2.z + z);
  ...
  for (int i = 0; i < points_count; i++) {
    ...
    this->faces.push_back(Polygon(std::vector<Vertex>(
        {Vertex(vertices.size() - 1, 2), Vertex(points_count * (2 * layers_count - 1) + i + 1, 0),
...
void Sphere::set_r(GLfloat new_r) {
  if (new_r <= 0) {
    throw std::invalid_argument("new_r must be > 0");
  }
...
void Sphere::set_a(GLfloat new_a) {
  if (new_a <= 0) {
    throw std::invalid_argument("new_a must be > 0");
  }
...
void Sphere::set_b(GLfloat new_b) {
  if (new_b <= 0) {
    throw std::invalid_argument("new_b must be > 0");
  }
...
void Sphere::set_c(GLfloat new_c) {
  if (new_c <= 0) {
    throw std::invalid_argument("new_c must be > 0");
  }
