...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
  ...
  } else if (key == GLFW_KEY_A && mods == GLFW_MOD_SHIFT && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    sphere.set_a(sphere.get_a() + 0.1);
    sphere.rebuild();
  } else if (key == GLFW_KEY_A && mods == GLFW_MOD_CONTROL && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    if (sphere.get_a() > 1) {
      sphere.set_a(sphere.get_a() - 0.1);
    }
    sphere.rebuild();
  } else if (key == GLFW_KEY_B && mods == GLFW_MOD_SHIFT && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    sphere.set_b(sphere.get_b() + 0.1);
    sphere.rebuild();
  } else if (key == GLFW_KEY_B && mods == GLFW_MOD_CONTROL && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    if (sphere.get_b() > 1) {
      sphere.set_b(sphere.get_b() - 0.1);
    }
    sphere.rebuild();
  } else if (key == GLFW_KEY_C && mods == GLFW_MOD_SHIFT && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    sphere.set_c(sphere.get_c() + 0.1);
    sphere.rebuild();
  } else if (key == GLFW_KEY_C && mods == GLFW_MOD_CONTROL && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
    if (sphere.get_c() > 1) {
      sphere.set_c(sphere.get_c() - 0.1);
    }
    sphere.rebuild();
