...
void InitSingleLinkedList(Elem **L) { (*L) = NULL; }
...
int ListEmpty(Elem *L) { return L->next == NULL ? 1 : 0; }

void InsertHead(Elem **L, char *data) {
  int len = strlen(data) + 1;
  char *word = malloc(len);
  for (int i = 0; i < len; i++) word[i] = data[i];
...
char *Delete(Elem **L) {
  ...
  char *word = malloc(len);
  for (int i = 0; i < len; i++) word[i] = prev->word[i];
  (*L) = (*L)->next;
  free(prev->word);
  free(prev);
  return word;
