...
void csort(char *src, char *dest)
{
    ...
    while(j<N-1){
        int i = j+1;
        while(i<N){
            if(wordlen[i]<wordlen[j]) count[j]+=wordlen[i]+1;
            else count[i]+=wordlen[j]+1;
            i++;
        }
        j++;
    }
    ...
    free(count);
    free(wordlen);
    //for(int i = 0; i < N; i++) printf("%d ", wordlen[i]);
    //printf("%s", dest);
    //return count[0];
}
