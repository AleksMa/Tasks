...
int ListEmpty(struct DoubleLinkedList *L){
    return L->next==L?1:0;
}
...
//Пусть неотсортированная часть последовательности - та "часть" списка, которая еще не входит в список
//Тогда каждый элемент списка можно ставить на правильное место в отсортированной последовательности, которая изначально пуста (значение ограничителя примем наименьшим int'ом)
